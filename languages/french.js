...
module.exports = {
    ...
    utils: {
        ...
        viewConf: () => `[Voir la configuration sur le dashboard](https://delta-bot.com)`,
        conf: {
            title: () => `Voir la configuration sur le dashboard`,
            content: () => `[ou sur le dashboard](https://delta-bot.com)`
        },
        specialMessages: {
            join: {
    ...
    help: {
        ...
        // Ranks
        ranks: {
            title: () => `:gift: Commandes récompenses`,
            content: (prefix) => "`"+ prefix +"addrank`,`"+ prefix +"removerank`,`"+ prefix +"ranks`"},
        // Join DM Messages
        joinDM: {
            title: () => `:envelope_with_arrow: Messages d'arrivées en MP`,
            content: (prefix) => "`"+ prefix +"configdm`,`"+ prefix +"setdm`,`"+ prefix +"testdm`"},
        // Join Messages
        join: {
            title: () => `:incoming_envelope: Messages d'arrivées`,
        ...
        links: (clientID) => `[Ajoutez-moi](https://discordapp.com/api/oauth2/authorize?client_id=${clientID}&permissions=8&scope=bot) ● [SiteWeb](https://delta-bot.com) ● [Serveur de support](${discord}) ● [Votez pour moi](https://discord.boats/bot/${clientID}/vote) ● [Twitter](https://twitter.com/DeltaBotInc)`
    },
    ...
    botinfos: {
        ...
        statistics: {
            title: () => `📊 Statistiques`,
            content: (guilds, users, channels) => `\`Serveurs: ${guilds}\`\n\`Salons: ${channels}\`\n\`Utilisateurs: ${users}\``
        },
        // Versions
        versions: {
            title: () => `⚙️ Versions`,
    ...
    support: {
        content: () => `:information_source: Si vous avez des questions ou si vous avez besoin de plus d'informations, vous pouvez rejoindre DeltaBot Inc. :\n${discord}`
    },
...
:pencil: **| Écrivez maintenant le message de départs... :pencil2:**`,
    ...
    ranks: {
        ...
        description: () => `Voici les récompenses d'invitations`
    },
